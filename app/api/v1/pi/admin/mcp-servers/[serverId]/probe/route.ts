...
export async function POST(request: Request, { params }: { params: Promise<{ serverId: string }> }) {
  try {
    ...
    const server = await getPiMcpRegistry().probeCapabilities(context, serverId, version);
    const tools = server.tools.map((tool) => {
      const { inputSchema, ...safeTool } = tool;
      void inputSchema;
      return safeTool;
    });
    return NextResponse.json({ data: { id: server.id, version: server.version, schemaDigest: server.schemaDigest, tools, probedAt: server.probedAt }, meta: { traceId: context.traceId } }, { status: 202 });
  } catch (error) {
