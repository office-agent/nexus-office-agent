import { NextResponse } from "next/server";
import { getPiModelGateway } from "@/src/modules/pi-agent/runtime";
import { piModelRouteActionSchema } from "@/src/modules/pi-agent/application/schemas";
import { presentModelRoute } from "@/src/modules/pi-agent/application/m30-presenter";
import { resolveRequestContext } from "@/src/platform/context/resolve-request-context";
import { applicationErrorResponse, parseJson, requireIdempotencyKey } from "@/src/platform/http/api-response";

export const dynamic = "force-dynamic";

export async function POST(request: Request, { params }: { params: Promise<{ routeId: string }> }) {
  try {
    const context = await resolveRequestContext(request);
    requireIdempotencyKey(request);
    const { routeId } = await params;
    const { version } = piModelRouteActionSchema.parse(await parseJson(request));
    const route = await getPiModelGateway().approveRoute(context, routeId, version);
    return NextResponse.json({ data: presentModelRoute(route), meta: { traceId: context.traceId } });
  } catch (error) {
    return applicationErrorResponse(error);
  }
}
