...
export async function POST(request: Request, { params }: { params: Promise<{ rolloutId: string }> }) {
  try { const context = await resolveRequestContext(request); requireIdempotencyKey(request); const { rolloutId } = await params; const rollout = await getPiReleaseGovernanceService().rollbackRollout(context, rolloutId); return NextResponse.json({ data: presentRollout(rollout), meta: { traceId: context.traceId } }); } catch (error) { return applicationErrorResponse(error); }
}
