...
export async function GET(request: Request) {
  try {
    ...
    const service = getTaskCommandService();
    const stream = new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        const poll = async () => {
          if (closed) return;
          try {
            const events = await service.messageEvents(context, cursor, 100);
            for (const item of events) {
              cursor = item.sequence;
              send(`id: ${item.sequence}\nevent: message-change\ndata: ${JSON.stringify(item)}\n\n`);
            }
          } catch {
            send('event: stream-error\ndata: {"code":"MESSAGE_STREAM_READ_FAILED"}\n\n');
            close();
            return;
          }
